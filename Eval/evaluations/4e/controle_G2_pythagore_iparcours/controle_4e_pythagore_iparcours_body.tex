% Contenu partagÃ© (Ã©lÃ¨ve/corrigÃ©) â€” IParcours G2
% !TeX program = lualatex

% BarÃ¨me indicatif (ajustez si besoin)
\bareme{
  \exbadge{1}{5} \quad
  \exbadge{2}{4} \quad
  \exbadge{3}{3}
}

% =============================
% EXERCICE 1 â€” Cours et propriÃ©tÃ©s (8 pts)
% =============================
\begin{exercisebox}{\faIcon{book} Exercice 1 : Cours et propriÃ©tÃ©s \points{5}}
  \textbf{1.} Ã‰nonce le \textbf{thÃ©orÃ¨me de Pythagore}. \points{1}

  \anslines[0.5cm]{3}{\begin{minipage}{\linewidth}
    Dans un triangle rectangle, le carrÃ© de la longueur de l'hypotÃ©nuse est Ã©gal Ã  la somme des carrÃ©s des longueurs des deux autres cÃ´tÃ©s.
  \end{minipage}}

  \smallspace

  \textbf{2.} ComplÃ¨te chaque propriÃ©tÃ© avec lâ€™Ã©galitÃ© de Pythagore correspondante. \points{2}
  \begin{enumerate}[label=\alph*)]
    \item Si un triangle $FQZ$ est rectangle en $Q$ alors\;\onlyeleve{\underline{\hspace{2.2cm}} + \underline{\hspace{2.2cm}} = \underline{\hspace{2.2cm}}} \onlycorrige{$\;FZ^2 = FQ^2 + QZ^2$}.
    \item Si un triangle $JMX$ est rectangle en $X$ alors\;\onlyeleve{\underline{\hspace{2.2cm}} + \underline{\hspace{2.2cm}} = \underline{\hspace{2.2cm}}} \onlycorrige{$\;JM^2 = JX^2 + XM^2$}.
    \item Si un triangle $KNS$ est rectangle en $K$ alors\;\onlyeleve{\underline{\hspace{2.2cm}} + \underline{\hspace{2.2cm}} = \underline{\hspace{2.2cm}}} \onlycorrige{$\;NS^2 = NK^2 + KS^2$}.
    \item Si un triangle $PIO$ est rectangle en $I$ alors\;\onlyeleve{\underline{\hspace{2.2cm}} + \underline{\hspace{2.2cm}} = \underline{\hspace{2.2cm}}} \onlycorrige{$\;PO^2 = PI^2 + IO^2$}.
  \end{enumerate}

  \smallspace

  \textbf{3.} Ã‰nonce la \textbf{rÃ©ciproque du thÃ©orÃ¨me de Pythagore}. \points{1}

  \anslines[0.5cm]{3}{\begin{minipage}{\linewidth}
    Si, dans un triangle, le carrÃ© de la longueur du plus grand cÃ´tÃ© est Ã©gal Ã  la somme des carrÃ©s des longueurs des deux autres cÃ´tÃ©s, alors ce triangle est rectangle (lâ€™angle droit est Ã  lâ€™extrÃ©mitÃ© commune aux deux petits cÃ´tÃ©s).
  \end{minipage}}

  \smallspace

  \textbf{4.} ComplÃ¨te chaque propriÃ©tÃ© (utiliser la rÃ©ciproque). \points{1}
  \begin{enumerate}[label=\alph*)]
    \item Si $ST^2 + TJ^2 = SJ^2$ alors le triangle \onlyeleve{\underline{\hspace{1.6cm}}} \onlycorrige{$\;STJ$} est rectangle en \onlyeleve{\underline{\hspace{1cm}}} \onlycorrige{$\;T$}.
    \item Si $MR^2 + MV^2 = RV^2$ alors le triangle \onlyeleve{\underline{\hspace{1.6cm}}} \onlycorrige{$\;MRV$} est rectangle en \onlyeleve{\underline{\hspace{1cm}}} \onlycorrige{$\;M$}.
    \item Si $WX^2 + XY^2 = WY^2$ alors le triangle \onlyeleve{\underline{\hspace{1.6cm}}} \onlycorrige{$\;WXY$} est rectangle en \onlyeleve{\underline{\hspace{1cm}}} \onlycorrige{$\;X$}.
  \end{enumerate}
\end{exercisebox}

\exspace

% =============================
% EXERCICE 2 â€” VÃ©rifier si un triangle est rectangle (4 pts)
% =============================
\begin{exercisebox}{\faIcon{check-double} Exercice 2 : Triangle rectangle ? \points{4}}
  \textbf{7.} Soit $BUT$ un triangle tel que $TU = \SI{5,1}{cm}$, $BU = \SI{7,9}{cm}$ et $BT = \SI{6}{cm}$. Ce triangle est-il rectangle ? Justifie ta rÃ©ponse. \points{2}

  \anslines[0.55cm]{6}{\begin{minipage}{\linewidth}
    $\max(BU, BT, TU) = BU$. On compare $BU^2$ et $BT^2 + TU^2$ :\\
    $BU^2 = 7{,}9^2 = 62{,}41$,\quad $BT^2 + TU^2 = 6^2 + 5{,}1^2 = 36 + 26{,}01 = 62{,}01$.\\
    Comme $62{,}41 \neq 62{,}01$, on nâ€™a pas lâ€™Ã©galitÃ© de Pythagore. Donc le triangle $BUT$ nâ€™est \textbf{pas} rectangle.
  \end{minipage}}

  \smallspace

  \textbf{8.} Soit $CAR$ un triangle tel que $RA = \SI{3,3}{cm}$, $AC = \SI{5,6}{cm}$ et $RC = \SI{6,5}{cm}$. Ce triangle est-il rectangle ? Justifie ta rÃ©ponse. \points{2}

  \anslines[0.55cm]{6}{\begin{minipage}{\linewidth}
    $\max(RC, RA, AC) = RC$. On compare $RC^2$ et $RA^2 + AC^2$ :\\
    $RA^2 + AC^2 = 3{,}3^2 + 5{,}6^2 = 10{,}89 + 31{,}36 = 42{,}25$;\quad $RC^2 = 6{,}5^2 = 42{,}25$.\\
    Les deux quantitÃ©s sont Ã©gales, donc par la rÃ©ciproque de Pythagore, $CAR$ est rectangle en $A$.
  \end{minipage}}
\end{exercisebox}

\exspace

% =============================
% EXERCICE 3 â€” ProblÃ¨me (vie courante) : Table de chevet (3 pts)
% =============================
\begin{problembox}{\faIcon{cubes} Exercice 3 : La table de chevet \points{3}}
  \noindent\begin{minipage}[t]{0.56\linewidth}
    \vspace*{-0.6\baselineskip}% aligne visuellement avec la figure
  Un designer a imaginÃ© une table de chevet en marbre (plateau) et mÃ©tal (structure). On modÃ©lise la structure ci-dessous. Les segments \([JL]\) et \([KI]\) sont \textbf{perpendiculaires} en \(M\).


  \end{minipage}\hfill
  \begin{minipage}[t]{0.40\linewidth}
    \centering
    \def\chevetimg{../images/fig_table_chevet_controle_pythagore}
    \IfFileExists{\chevetimg.pdf}{\includegraphics[width=\linewidth]{\chevetimg.pdf}}{%
      \IfFileExists{\chevetimg.png}{\includegraphics[width=\linewidth]{\chevetimg.png}}{%
        \IfFileExists{\chevetimg.jpg}{\includegraphics[width=\linewidth]{\chevetimg.jpg}}{%
          \begin{tikzpicture}[x=1mm,y=1mm]
            \draw[gray] (0,0) rectangle (70,45);
            \node at (35,22.5) {\small Image manquante};
          \end{tikzpicture}%
        }%
      }%
    }
  \end{minipage}
  

  \textbf{Questions.}
  \begin{enumerate}[label=\arabic*.]
    \item Calcule les longueurs \(JK\) et \(JI\). \points{2}
      \anslines[0.6cm]{4}{\onlycorrige{\(JK = \sqrt{JM^2+MK^2} = \sqrt{38{,}4^2+28{,}8^2} = \sqrt{2304} = \SI{48}{cm}.\\
      JI = \sqrt{JM^2+MI^2} = \sqrt{38{,}4^2+51{,}2^2} = \sqrt{4096} = \SI{64}{cm}.\)}}

    \item En dÃ©duire si le montant \([JK]\) est vertical et si le plateau \([JI]\) est parallÃ¨le au sol. \points{1}
      \anslines[0.6cm]{4}{\onlycorrige{On a \(KI = KM + MI = 28{,}8 + 51{,}2 = \SI{80}{cm}.\)
      Dans le triangle \(JKI\), \(JK^2 + JI^2 = 48^2 + 64^2 = 2304 + 4096 = 6400 = 80^2 = KI^2\). \\
      Donc lâ€™angle \(\widehat{KJI}\) est droit: \(JK \perp JI\). Ainsi, \(JK\) est vertical et \(JI\) est horizontal (donc parallÃ¨le au sol).}}
  \end{enumerate}
\end{problembox}

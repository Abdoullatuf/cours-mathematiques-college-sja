...
\item Les points $X$, $Y$ et $Z$ sont alignés : \ans{$X \in (YZ)$ ou les trois points appartiennent à la même droite}
\end{itemize}
}
...
% Points
\node[circle,fill=black,inner sep=1.5pt,label=above:$A$] (A) at (1,0) {};
\node[circle,fill=black,inner sep=1.5pt,label=above:$B$] (B) at (3,0) {};
\node[circle,fill=black,inner sep=1.5pt,label=above:$C$] (C) at (2,1) {};
\node[circle,fill=black,inner sep=1.5pt,label=above:$D$] (D) at (0.5,2.25) {};
\node[circle,fill=black,inner sep=1.5pt,label=above:$E$] (E) at (1.5,1.5) {};
\end{tikzpicture}
\end{center}
...
$A$ \ans{$\in$} $(d_1)$ \quad ; \quad $C$ \ans{$\notin$} $(d_2)$ \quad ; \quad $B$ \ans{$\notin$} $(d_2)$ \quad ; \quad $D$ \ans{$\notin$} $(d_1)$
...
\anslines{2}{Oui, les points C, D et E sont alignés car ils appartiennent tous les trois à la droite $(d_2)$.}
\end{exercisebox}
...
\ans{se coupent en formant un angle droit}
\end{itemize}
\end{exercisebox}
...
\onlycorrige{\vspace{0.3cm}\textcolor{warning}{\textbf{Barème :} 1 point par construction correcte et précise.}}
\end{constructionbox}

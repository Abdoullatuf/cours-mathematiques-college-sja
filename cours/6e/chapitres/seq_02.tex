...
\chapter{Points, droites et programmes de construction}
\label{chap:seq02}
...
\begin{itemize}
	...
	La droite passant par $A$ et $B$ se note : \trou{3.8cm}.
	\item Un \trou{2.2cm} est une portion de \trou{2.2cm} délimitée par deux \trou{2.2cm} appelés \trou{2.2cm}.
	On le note : \trou{3cm}.
	\item Une \trou{2.2cm} est une partie de \trou{2.2cm} qui commence en un \trou{2.5cm} donné et s'étend à l'infini.
	On la note par exemple : \trou{2.8cm}.
\end{itemize}
...
\textbf{Lecture :} $(AB)//(EF)$ se lit << \trou{5cm} >>.\\
$(d)\perp(\Delta)$ se lit << \trou{5cm} >>.
...
\begin{figure}[h]
	...
	%\caption{Vocabulaire : $(AB)$ droite ; $[CD]$ segment ; $[EF)$ demi-droite ; $AB$ longueur.}
	\label{fig:position-relative-droites}
\end{figure}
...
% --- Deux propriétés : perpendiculaire et parallèle
\begin{proprietebox}[Deux propriétés]
\begin{itemize}[label = \textbullet]
    \item Si deux droites sont perpendiculaires à une même droite, alors elles sont parallèles.
    \item Si deux droites sont parallèles et si une troisième droite est perpendiculaire à l'une, alors elle est perpendiculaire à l'autre.
\end{itemize}
...
\begin{tikzpicture}[scale=1]
    % Droite (d)
    \draw[dro] (0,-1) -- (0,1) node[below left] {$(d)$};
    % Deux droites parallèles perpendiculaires à (d)
    \draw[para] (-2,0.5) -- (2,0.5) node[right] {$(d_1)$};
    \draw[para] (-2,-0.5) -- (2,-0.5) node[right] {$(d_2)$};
    % Marques d'angle droit
    \draw[line width=0.6pt] (0,0.5) -- (0.3,0.5) -- (0.3,0.2);
    \draw[line width=0.6pt] (0,-0.5) -- (0.3,-0.5) -- (0.3,-0.8);
\end{tikzpicture}
\end{center}
\end{proprietebox}

\clearpage
\section{Application : Lire attentivement une consigne}
...
\begin{itemize}
	\item[\textbullet] Lecture : $[MN]$: \trous{9cm}\\
	 $(RS)$: \trous{9cm}\\
	 $[TU)$: \trous{9cm}\\
	 $VU$: \trous{9cm}
	\item[\textbullet] Notations : << La droite passant par $P$ et $Q$ >>: \trous{9cm}\\
	 << Le segment d'extrémités $K$ et $L$ >>: \trous{9cm} \\
	 << La demi-droite d'origine $H$ passant par $J$ >>: \trous{9cm} \\
	 << La longueur du segment d'extrémités A et B >>: \trous{9cm}
	\item[\textbullet] Complète :\\
	Si $A \in (BC)$ alors $A,B,C$ sont \trou{3cm}. Si $D \notin (EF)$ alors D, E et F \trou{5cm}
\end{itemize}

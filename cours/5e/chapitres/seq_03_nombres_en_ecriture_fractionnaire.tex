...
\begin{activitybox}[Manipulation : Le Partage Équitable des Bandes - Partie 1]
	...
	\begin{itemize}
		...
		\item Des bandes à mesurer : 10,5 cm ; \textbf{7 cm} ; 15,75 cm ; 5,25 cm
	...
	\begin{remarkbox}
		\textbf{$\triangleright$ Piste :}

		Si le pliage ne fonctionne pas pour toutes les bandes (notamment celle de \textbf{7 cm}), quelle autre méthode pourriez-vous utiliser ? Pensez à reporter la longueur de la bande unité sur une bande vierge pour trouver le bon partage.
	\end{remarkbox}
	...
	\begin{itemize}
		\item « J'ai plié ma bande unité en \solution[3cm]{2, 3, 4 ou 8} parties égales »
		\item « J'ai pris \solution[3cm]{1, 2, 3...} de ces parties »
		\item « Ma bande mesure \solution[5cm]{$\frac{1}{2}$, $\frac{1}{3}$, $\frac{3}{4}$...} de l'unité »
	\end{itemize}

	\medskip

	\begin{astucebox}
		\textbf{Question de relance :}
		...
		\solution[8cm]{Car 2 et 4 sont des puissances de 2, on peut plier facilement. Pour 3, le pliage ne fonctionne pas directement.}
	\end{astucebox}
\end{activitybox}

\clearpage

\begin{activitybox}[Manipulation : Le Partage Équitable des Bandes - Partie 2]
	...
	\begin{center}
		\begin{tabular}{|c|c|c|}
			...
			\hline
			A & \solution[3cm]{10,5 cm} & \solution[3cm]{$\frac{1}{2}$} \\
			\hline
			B & \solution[3cm]{7 cm} & \solution[3cm]{$\frac{1}{3}$} \\
			\hline
			C & \solution[3cm]{15,75 cm} & \solution[3cm]{$\frac{3}{4}$} \\
			\hline
		\end{tabular}
	\end{center}
	...
	\solution[10cm]{Oui, par exemple $\frac{5}{4}$, $\frac{7}{3}$, $\frac{11}{2}$... Le numérateur est plus grand que le dénominateur.}
\end{activitybox}
...
\begin{definitionbox}[Écriture fractionnaire]
	\begin{minipage}[t]{0.62\textwidth}
		...
		\begin{itemize}
			\item Le nombre du haut, $a$, s'appelle le \textbf{\solution[3cm]{numérateur}}.
			\item Le nombre du bas, $b$, s'appelle le \textbf{\solution[3.5cm]{dénominateur}}.
			\item Le trait de fraction signifie « \solution[3cm]{divisé par} ».
		\end{itemize}
	\end{minipage}%
	\hfill
...
\begin{remarkbox}
	\begin{itemize}
		\item On lit $\frac{3}{4}$ : « \solution[3cm]{trois quarts} » ou « trois sur quatre ».
		\item Une fraction représente le \textbf{\solution[2.5cm]{quotient}} : $\frac{a}{b} = a \div b$.
		\item Le dénominateur ne peut \textbf{jamais} être égal à \solution[1.5cm]{zéro} (on ne peut pas diviser par zéro).
	\end{itemize}
\end{remarkbox}
...
\begin{examplebox}
	...
	\begin{itemize}
		\item \textbf{Partage :} On prend \solution[1cm]{5} parts d'un objet découpé en \solution[1cm]{2} parts égales.
		\item \textbf{Quotient :} $\frac{5}{2} = 5 \div 2 = \solution[1.5cm]{2,5}$
		\item \textbf{Fraction supérieure à 1 :} $\frac{5}{2} = \frac{\solution[1cm]{4} + \solution[1cm]{1}}{\solution[1cm]{2}} = \frac{\solution[1cm]{4}}{\solution[1cm]{2}} +\frac{\solution[1cm]{1}}{\solution[1cm]{2}} = \solution[1cm]{2} + \frac{\solution[1cm]{1}}{\solution[1cm]{2}}$
	\end{itemize}
...
\begin{activitybox}[Observer des fractions égales]
	...
	\textbf{Que remarquez-vous ?} \solution[12cm]{Les trois rectangles ont la même partie colorée, donc les fractions sont égales.}

	\medskip

	\begin{methodebox}[Analyse des transformations]
		\begin{enumerate}
			\item \textbf{Question 1 :} Pour passer du rectangle représentant $\frac{1}{2}$ à celui de $\frac{2}{4}$, par combien a-t-on multiplié le nombre total de parts ? Par combien a-t-on multiplié le nombre de parts colorées ?

			\solution[8cm]{On a multiplié par 2 le nombre de parts (2 → 4) et par 2 le nombre de parts colorées (1 → 2).}

			\item \textbf{Question 2 :} Comment peut-on passer de l'écriture $\frac{1}{2}$ à l'écriture $\frac{2}{4}$ en utilisant la même opération sur le nombre du haut (numérateur) et le nombre du bas (dénominateur) ?

			\solution[8cm]{On multiplie le numérateur ET le dénominateur par 2 : $\frac{1 \times 2}{2 \times 2} = \frac{2}{4}$}
		\end{enumerate}
	\end{methodebox}
	...
	\begin{center}
		$\frac{2}{4} = $ \solution[2cm]{0,5} \qquad $\frac{4}{8} = $ \solution[2cm]{0,5} \qquad $\frac{1}{2} = $ \solution[2cm]{0,5}
	\end{center}

	\textbf{Conclusion :} \solution[12cm]{Les trois fractions sont égales car elles donnent le même résultat : $\frac{2}{4} = \frac{4}{8} = \frac{1}{2} = 0,5$}
\end{activitybox}

\subsection{Règle fondamentale}

\begin{proprietebox}[Fractions égales]
	On ne change pas la valeur d'une fraction en \textbf{\solution[3cm]{multipliant}} ou en \textbf{\solution[2.5cm]{divisant}} son numérateur et son dénominateur par un \textbf{même nombre non nul}.

	$$\frac{a}{b} = \frac{a \times \solution[1cm]{k}}{b \times \solution[1cm]{k}} \qquad \text{et} \qquad \frac{a}{b} = \frac{a \div \solution[1cm]{k}}{b \div \solution[1cm]{k}}$$
...
\begin{examplebox}
	...
	\begin{minipage}[t]{0.48\textwidth}
		$$\frac{3}{5} = \frac{3 \times 4}{5 \times 4} = \frac{\solution[1cm]{12}}{\solution[1cm]{20}}$$
	\end{minipage}
	\hfill
	\begin{minipage}[t]{0.48\textwidth}
		$$\frac{18}{24} = \frac{18 \div 6}{24 \div 6} = \frac{\solution[1cm]{3}}{\solution[1cm]{4}}$$
	\end{minipage}
	...
	$$\frac{2}{3} = \frac{2 \times \solution[1cm]{2}}{3 \times \solution[1cm]{2}} = \frac{\solution[1cm]{4}}{\solution[1cm]{6}} = \frac{2 \times \solution[1cm]{3}}{3 \times \solution[1cm]{3}} = \frac{\solution[1cm]{6}}{\solution[1cm]{9}} = \frac{2 \times \solution[1cm]{5}}{3 \times \solution[1cm]{5}} = \frac{\solution[1cm]{10}}{\solution[1cm]{15}}$$
\end{examplebox}

\section{Simplification de fractions}

\begin{definitionbox}[Simplifier une fraction]
	\textbf{Simplifier} une fraction, c'est trouver une fraction \solution[2cm]{égale} ayant un numérateur et un dénominateur plus \solution[2cm]{petits}.

	On dit qu'une fraction est \textbf{\solution[3cm]{irréductible}} (ou \textbf{simplifiée au maximum}) lorsque le numérateur et le dénominateur n'ont plus de diviseur commun autre que \solution[1cm]{1}.
\end{definitionbox}

\begin{methodebox}[Simplifier une fraction]
	Pour simplifier une fraction, on peut utiliser deux approches :

	\textbf{1. Méthode « pas à pas » (toujours possible) :}
	\begin{itemize}
		\item Chercher un \solution[4cm]{diviseur commun} simple (2, 3, 5, 10...) au numérateur et au dénominateur en utilisant les critères de divisibilité.
		\item \solution[6cm]{Diviser} le numérateur et le dénominateur par ce diviseur.
		\item \solution[8cm]{Recommencer} avec la nouvelle fraction obtenue, jusqu'à ce qu'il n'y ait plus de diviseur commun (à part 1).
	\end{itemize}

	\textbf{2. Méthode « la plus efficace » (plus rapide) :}
	\begin{itemize}
		\item Chercher le \solution[8cm]{plus grand diviseur commun} au numérateur et au dénominateur.
		\item \solution[6cm]{Diviser} directement le numérateur et le dénominateur par ce plus grand diviseur.
		\item La fraction est alors immédiatement \solution[3cm]{irréductible}.
	\end{itemize}
\end{methodebox}
...
\begin{examplebox}
	...
	\textbf{Méthode 1 :} Simplifications successives (« pas à pas »)
	\begin{align*}
		\frac{24}{36} &= \frac{24 \div 2}{36 \div 2} = \frac{\solution[1cm]{12}}{\solution[1cm]{18}} \quad \text{(divisé par 2)}\\
		&= \frac{12 \div 2}{18 \div 2} = \frac{\solution[1cm]{6}}{\solution[1cm]{9}} \quad \text{(divisé par 2)}\\
		&= \frac{6 \div 3}{9 \div 3} = \frac{\solution[1cm]{2}}{\solution[1cm]{3}} \quad \text{(divisé par 3)}
	\end{align*}

	\textbf{Méthode 2 :} Simplification directe (« la plus efficace »)

	\textit{On cherche le plus grand nombre qui divise à la fois 24 et 36. C'est \solution[1.5cm]{12}.}
	$$\frac{24}{36} = \frac{24 \div \solution[1cm]{12}}{36 \div \solution[1cm]{12}} = \frac{\solution[1cm]{2}}{\solution[1cm]{3}}$$

	La fraction $\frac{2}{3}$ est \solution[3cm]{irréductible}.
\end{examplebox}

\begin{astucebox}
	\textbf{Astuces pour simplifier rapidement on utilise les critères de divisibilités :}
	\begin{itemize}
...
\begin{examplebox}
	...
	\begin{astucebox}
		\textbf{$\triangleright$ Astuce : Du français aux maths}

		En mathématiques, le mot \textbf{« de »} se traduit très souvent par une \textbf{multiplication (×)}.

		Calculer « $\frac{3}{4}$ \textbf{de} 20 € » revient donc à faire le calcul : $\frac{3}{4} \times 20$.
...
\begin{examplebox}
	...
	\begin{center}
		\begin{tikzpicture}[scale=1.2]
			...
			\node[above,mainRed] at (3.5,0.5) {$\frac{7}{4}$};
			\draw[mainRed,thick,->,line width=1pt] (3.5,0.35) -- (3.5,0.12);
		\end{tikzpicture}
	\end{center}
\end{examplebox}
...
\begin{exercisebox}
	...
	\begin{multicols}{2}
		\begin{enumerate}[label=\alph*)]
			\item $\frac{3}{7} = \frac{\solution[1cm]{6}}{14}$
			\item $\frac{5}{8} = \frac{15}{\solution[1cm]{24}}$
			\item $\frac{12}{18} = \frac{\solution[1cm]{2}}{3}$
			\item $\frac{20}{35} = \frac{4}{\solution[1cm]{7}}$
			\item $\frac{2}{5} = \frac{\solution[1cm]{40}}{100}$
			\item $\frac{9}{12} = \frac{3}{\solution[1cm]{4}}$
		\end{enumerate}
	\end{multicols}
...
\begin{exercisebox}
	...
	\textbf{g) Justifier} (Nouvelle question)
	...
	\ifcorrige
	{\color{mainBlue}
	\textbf{Réponse :} Les deux fractions ont le même numérateur (2). Quand on partage en 5 parts, chaque part est plus petite que quand on partage en 3 parts seulement. Donc 2 parts sur 5 est plus petit que 2 parts sur 3.

	\textit{Schéma :} Dessiner deux barres identiques, l'une divisée en 5 (colorier 2 parts), l'autre en 3 (colorier 2 parts). On voit que 2/3 est plus grand.
	}
	\else
	\makebox[\textwidth]{\rule{0pt}{5cm}\dotfill}
	\fi
...
\begin{exercisebox}[Remédiation] ($\star$ Pour les élèves en difficulté)
	...
	\begin{enumerate}[label=\alph*)]
		\item $\frac{2}{3} = \frac{4}{\solution[1cm]{6}} = \frac{6}{\solution[1cm]{9}} = \frac{\solution[1cm]{8}}{12}$
		\item $\frac{1}{2} = \frac{2}{\solution[1cm]{4}} = \frac{3}{\solution[1cm]{6}} = \frac{5}{\solution[1cm]{10}}$
	\end{enumerate}
...
\begin{exercisebox}[Approfondissement] ($\star\star\star$ Pour aller plus loin)
	...
	\textbf{Exercice Bonus : Trouve l'erreur !} ($\star\star$ Raisonnement)

	Thomas affirme : « Pour simplifier une fraction, je divise le numérateur par 2 et le dénominateur par 3 ».

	\begin{enumerate}[label=\alph*)]
		\item Que penses-tu de cette méthode ?

		\solution[5cm]{Cette méthode est fausse ! On doit diviser par le MÊME nombre.}

		\item Donne un exemple qui montre que Thomas se trompe.

		\solution[5cm]{$\frac{6}{9}$ : si on fait 6÷2 = 3 et 9÷3 = 3, on obtient $\frac{3}{3} = 1$. Mais $\frac{6}{9} = \frac{2}{3} \neq 1$.}

		\item Quelle est la règle correcte pour simplifier une fraction ?

		\solution[5cm]{Il faut diviser le numérateur ET le dénominateur par le MÊME nombre (diviseur commun).}
	\end{enumerate}
\end{exercisebox}